...
$id = $name  = $mk =  $status =$manv=$diachi=$sdt=$hoten= '';
if (!empty($_POST)) {
	...
	if (!empty($name)) {
		//Luu vao database
		if ($id == '') {
			   $sql = 'insert into login(taikhoan, matkhau,status,manv,diachi,sdt) values("'.$name.'", "'.$mk.'","'.$status.'","'.$manv.'","'.$diachi.'","'.$sdt.'","'.$hoten.'")';
		}
		 else {
			$sql = 'update login set taikhoan = "'.$name.'", matkhau = "'.$mk.'",status= "'.$status.'",manv= "'.$manv.'",diachi= "'.$diachi.'",sdt= "'.$sdt.'",hoten="'.$hoten.'" where id = '.$id;
		}
