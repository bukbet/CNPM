...
if (!empty($_POST)) {
	...
	if (!empty($title)) {
		...
		if ( (!isset($_GET['id'])) && $id == '') {
			...
			$sql = 'insert into post(tenbai, thumbnail, ngaydang, giatien, trangthai,   SDT, mota, id_user) values("'.$title.'", "'.$thumbnail.'",  "'.$created_at.'", "'.$price.'", "'.$status.'",   "'.$id_user.'")';
		} else {
			$id      = $_GET['id'];
			$sq     = 'select * from post where id = '.$id;
			$post = executeSingleResult($sq);
			$sql = 'update post set tenbai = "'.$title.'", thumbnail = "'.$thumbnail.'", noidung = "'.$content.'", ngaydang = "'.$created_at.'", giatien = "'.$price.'", trangthai = "'.$status.'", id_user = "'.$id_user.'" where id = '.$id;
		}
