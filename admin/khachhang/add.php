...
if (!empty($_POST)) {
	...
	if (!empty($hoten)) {
		//Luu vao database
		if ($id == '') {
			   $sql = 'insert into khach(hoten,diachi,sdt,date,time) values("'.$hoten.'", "'.$diachi.'","'.$sdt.'","'.$date.'","'.$time.'")';
		}
		 else {
			$sql = 'update khach set hoten= "'.$hoten.'", diachi = "'.$diachi.'",sdt= "'.$sdt.'"date= "'.$date.'",time= "'.$time.'" where id = '.$id;
		}
...
<body>
	<ul class="nav nav-tabs">
	  ...
	  <ul class="nav nav-tabs">
	  <li class="nav-item">
	    <a class="nav-link" href="index.php">Quản Lý Khách Hàng</a>
	  </li>
	</ul>
	...
	<div class="container">
		<div class="panel panel-primary">
			<div class="panel-heading">
				<h2 class="text-center">Thêm/Sửa Thông Tin Khách</h2>
			</div>
